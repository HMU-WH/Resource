...
############' var_counts: 突变位点与variant allele相匹配的reads数
############' normal_cn: 正常细胞该位点对应的拷贝数, 对于人类常染色体来说是2(性染色体除外)
...
##' @return 在"PyClone.Output.Dir"目录下输出运行结果文件:
############' config.yaml 用于PyClone分析的设置文件(与命令行参数设置有关)
############' yaml/ 存放yaml突变文件，用于PyClone分析(由传入的tsv文件转换而来) 
############' trace/ 包含MCMC抽样算法的迭代纪录即PyClone的分析过程纪录(与迭代次数的设置相关)
############' tables/ 包含PyClone的分析结果数据
############' plots/ 包含生成的全部图表(基于trace与tables中的文件绘制)
PyClone.Run <- function(PyClone.Input, 
  ...
  # 检查并判断System.PyClone.Alias在系统中是否存在
