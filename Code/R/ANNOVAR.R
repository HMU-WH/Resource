...
##' @param Database.Buildver character 注释数据库对应的基因组版本号, 可选("hg18", "hg19", "hg38"); 默认"hg18"
...
Annovar.Download <- function(Database, 
  ...
  # 检查并判断System.Perl.Alias在系统中是否存在
...
##' @param Annovar.Dir character Annovar的安装目录(必要pl文件所存在的目录); 默认NULL, 即当前工作目录
##' @param Database.Dir character 注释数据库信息的存放目录; 默认NULL, 即当前"Annovar.Dir"下的"humandb"目录
##' @param Database.Buildver character 基因组版本号, 可选("hg18", "hg19", "hg38"); 默认为"hg18"
##' @param Operation character[] 要进行的注释操作集合("g" for gene-based, "gx" for gene-based with cross-reference annotation, "r" for region-based and "f" for filter-based), 与"Database"一一对应; 默认NULL
##' @param Xref.File character 用于基因的交叉引用注释的文件(由制表符分隔, 第一列必须为gene名, 其余列为对应基因的其他注释信息, 文件如有列名, 首行前需添加"#'作为列名行的识别符), 若"Operation"不包含"gx", 则该参数将失去作用; 默认NULL
##' @param Do.Polish logical 是否要通过"encoding_change.pl"重新计算蛋白质序列来修饰indel蛋白质注释; 默认TRUE
##' @param Nastring character 设置对于缺失信息使用的填充符号, 默认为"."(对于vcf文件必须使用".", 因此当"Input.Type"为"vcf"时, 该参数将失去作用); 默认"."
##' @param Output.Prefix character 注释结果文件的前缀名[可携带路径]; 默认NULL, 即当前工作目录下的"Annovar.Result"
##' @param With.Otherinfo logical 对于传入的avinput文件, 注释结果是否保留前五列以外的信息, 默认FALS(当"Input.Type"为"vcf"时, 该参数将失去作用); 默认FALSE
##' @param Temp.Remove logical 是否删除注释过程中产生的临时文件; 默认TRUE
##' @param Csvout logical 注释结果是否以csv格式输出; 默认FALSE, 即默认结果是以制表符分隔的txt文件输出(对于vcf文件将输出转化后的avinput文件以及txt与vcf两种格式的结果文件, 因此当"Input.Type"为"vcf"时, 该参数将失去作用); 默认FALSE
##' @param System.Perl.Alias character Perl软件在系统中的可执行命令名; 默认"perl"
