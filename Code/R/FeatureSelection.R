...
Feature.Selection.Stepwise <- function(Data, Is.Reg = TRUE){
  ...
  stopifnot(length(Is.Reg) == 1)
  # 获取因变量在特征矩阵中的名称
  Category <- colnames(Data)[ncol(Data)]
  if(Is.Reg){
...
Feature.Selection.Cox <- function(Data, Time, Is.Event, Ties.Method = c("efron","breslow","exact"), Analysis.Mode = c("Single", "Multiple")){
  library(survival)
  # 参数处理
  ...
  stopifnot(nrow(Data) == length(Time) && nrow(Data) == length(Is.Event))
  # 要进行筛选的特征
  Features <- colnames(Data)
  # 构建生存对象——判断样本的的生存状态
  Sample.Status <- Surv(Time, Is.Event)
  # 对各特征进行单因素分析
